...
public ActorAddView(Stage stage, DataBase db) {
	this.stage=stage;
	this.db=db;
	controller = new ActorAddController(stage, db);
	main();
}
...
private void setAction() {
	...
	buttonAdd.setOnAction(e-> {
		...
		Actor actor = new Actor(textFieldName.getText(), textFieldName2.getText(), textFieldPseudo.getText(), comboBoxSex.getSelectionModel().getSelectedItem().toString(), hairColor, hairSize, textAreaNote.getText());
		
		controller.addingActorAction(actor);
	});
}
